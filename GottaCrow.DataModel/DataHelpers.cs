using JobSearch.Model;
using System;
using System.Collections.Generic;
using System.Data.Entity;
using System.Linq;

namespace GottaCrow.DAL
{
    public class DataHelpers
  {
    //public static void NewDbWithSeed() {
     
    //  Database.SetInitializer(new DropCreateDatabaseAlways<JobSearchDbContext>());
    //  using (var context = new JobSearchDbContext()) {
    //    if (context.Companies.Any()) {
    //      return;
    //    }
    //    var self = context.Companies.Add(new Company { Name = "MySelf" });
    //    var goodCompany = context.Companies.Add(new Company { Name = "A Good Company" });
    //    var bestCo = context.Companies.Add(new Company { Name = "The Best Company" });

    //    var person1 = new Ninja
    //    {
    //      Name = "JulieSan",
    //      ServedInOniwaban = false,
    //      DateOfBirth = new DateTime(1980, 1, 1),
    //      Clan = vtClan

    //    };
    //    var s = new Ninja
    //    {
    //      Name = "SampsonSan",
    //      ServedInOniwaban = false,
    //      DateOfBirth = new DateTime(2008, 1, 28),
    //      ClanId = 1

    //    };
    //    var l = new Ninja
    //    {
    //      Name = "Leonardo",
    //      ServedInOniwaban = false,
    //      DateOfBirth = new DateTime(1984, 1, 1),
    //      Clan = turtleClan
    //    };
    //    var r = new Ninja
    //    {
    //      Name = "Raphael",
    //      ServedInOniwaban = false,
    //      DateOfBirth = new DateTime(1985, 1, 1),
    //      Clan = turtleClan
    //    };
    //    context.Ninjas.AddRange(new List<Ninja> { j, s, l, r });
    //    var ninjaKC = new Ninja
    //    {
    //      Name = "Kacy Catanzaro",
    //      ServedInOniwaban = false,
    //      DateOfBirth = new DateTime(1990, 1, 14),
    //      Clan = amClan
    //    };
    //    var muscles = new NinjaEquipment
    //    {
    //      Name = "Muscles",
    //      Type = EquipmentType.Tool,

    //    };
    //    var spunk = new NinjaEquipment
    //    {
    //      Name = "Spunk",
    //      Type = EquipmentType.Weapon
    //    };

    //    ninjaKC.EquipmentOwned.Add(muscles);
    //    ninjaKC.EquipmentOwned.Add(spunk);
    //    context.Ninjas.Add(ninjaKC);

    //    context.SaveChanges();
    //    context.Database.ExecuteSqlCommand(
    //      @"CREATE PROCEDURE GetOldNinjas
    //                AS  SELECT * FROM Ninjas WHERE DateOfBirth<='1/1/1980'");

    //    context.Database.ExecuteSqlCommand(
    //       @"CREATE PROCEDURE DeleteNinjaViaId
    //                 @Id int
    //                 AS
    //                 DELETE from Ninjas Where Id = @id
    //                 RETURN @@rowcount");
    //  }
    //}
  }
}

